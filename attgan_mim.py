...
class Generator(nn.Module):
    def __init__(self, enc_dim=64, enc_layers=5, enc_norm_fn='batchnorm', enc_acti_fn='lrelu',
        ...
        ####### Added MIM contrastive loss ######
        self.feature_layer = nn.Linear(self.f_size * self.f_size * MAX_DIM, 128)
        #########################################
...
#         return zs
        ...
        ##### Add this line ####
        feature_repr = self.feature_layer(z.view(z.size(0), -1))
        return zs, feature_repr
    ...
    def forward(self, x, a=None, mode='enc-dec'):
        if mode == 'enc-dec':
            ...
            zs, _ = self.encode(x)  # Only pass zs to decode, discard feature_repr
            return self.decode(zs, a)
        if mode == 'enc':
...
def mim_loss(feature1, feature2, temperature=0.07):
    batch_size = feature1.size(0)
    feature1 = F.normalize(feature1, dim=1)
    feature2 = F.normalize(feature2, dim=1)
    
    logits = torch.matmul(feature1, feature2.t()) / temperature
...
class AttGAN():
    def __init__(self, args):
        ...
        #### Added this line 
        self.lambda_mim = args.lambda_min
...
#         g_loss = gf_loss + self.lambda_2 * gc_loss + self.lambda_1 * gr_loss
        #### Add this line ####
        mim_contrastive_loss = mim_loss(feat_a, feat_fake)
        g_loss = gf_loss + self.lambda_2 * gc_loss + self.lambda_1 * gr_loss + self.lambda_mim * mim_contrastive_loss
        ...
        errG = {
            ...
            'mim_loss': mim_contrastive_loss.item()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lambda_min', dest='lambda_min', type=float, default=1.0)
